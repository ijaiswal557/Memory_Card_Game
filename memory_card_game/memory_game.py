...
def resize_images(images_path, width,height):
    images = Image.open(images_path)
    image = images.resize((width,height), Image.Resampling.LANCZOS)
    return ImageTk.PhotoImage(image)

#load card images (replace with your image paths)
card_images = [
    resize_images('card1.jpg', 100, 100),
    resize_images('card2.jpg', 100, 100),
    resize_images('card3.jpg', 100, 100),
    resize_images('card4.jpg', 100, 100),
    resize_images('card5.jpg', 100, 100),
    resize_images('card6.jpg', 100, 100),
]
...
card_back = resize_images("card_back.jpg",100,100)
...
def create_cards():
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            card = canvas.create_image(x, y, image=card_images[i*cols+j], anchor='nw')
            cards.append({
                "id": card,
